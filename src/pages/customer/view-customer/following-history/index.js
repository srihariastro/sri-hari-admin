...
const FollowingHistory = ({ customerId }) => {
    const dispatch = useDispatch();
    const { followingHistoryByCustomerIdData } = useSelector(state => state?.customerReducer);

    //* Data-Table Column
    const columns = [
        { name: 'S.No.', selector: (row) => followingHistoryByCustomerIdData.indexOf(row) + 1, width: '80px' },
    ...
    useEffect(function () {
        //! Dispatching API for Getting Following History
        dispatch(CustomerActions.getFollowingHistoryByCustomerId({ customerId }));
    }, []);

    return (
        <>
            <MainDatatable data={followingHistoryByCustomerIdData} columns={columns} title={'Following History'} />
